...
function refreshAmpel() {
    ...
    xmlhttp.onreadystatechange = function() {
        if (this.readyState == 4 && this.status == 200) {
            ...
            clearAmpel();
            document.getElementById("state").innerHTML = "<b>Co2:</b> "+state.co2+" ppm<br><b>Temperature:</b> "+state.temp + " &ordm;C<br><b>Illumination:</b> "+state.lux+" LUX <br><b>Humidity:</b> "+state.hum+" %<br><b>MQTT connected:</b> "+!!state.mqtt+"<br><b>CO2 AutoCalibration:</b> "+(!!state.co2ASC)+"<br><br>Firmware: "+state.firmware;
            if (state.co2 < 800) {
